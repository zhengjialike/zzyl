...
public class MessageServiceImpl extends ServiceImpl<MessageMapper, Message> implements MessageService {
    ...
    @Override
    public PageResult<Message> findPage(int pageNum, int pageSize, String type, String startTime, String endTime, String isRead, Long receiverId) {
        Page<Message> page = new Page<>(pageNum, pageSize);
        LambdaQueryWrapper<Message> wrapper = new LambdaQueryWrapper<>();
        if (StringUtils.hasText(type)) {
            wrapper.eq(Message::getMsgType, type);
        }
    ...
    public void markAllAsRead(Long receiverId) {
        LambdaQueryWrapper<Message> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(Message::getReceiverId, receiverId).eq(Message::getIsRead, "未读");
        Message msg = new Message();
        msg.setIsRead(1);
        baseMapper.update(msg, wrapper);
    }
