...
public class ElderBalanceServiceImpl extends ServiceImpl<ElderBalanceMapper, ElderBalance> implements ElderBalanceService {
    ...
    @Override
    public PageResult<ElderBalance> findPage(int pageNum, int pageSize, String elderName, String bedNo) {
        ...
        if (StringUtils.hasText(elderName)) {
            wrapper.like(ElderBalance::getElderName, elderName);
        }
        if (StringUtils.hasText(bedNo)) {
            wrapper.like(ElderBalance::getBedNo, bedNo);
        }
